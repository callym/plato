...
impl Home {
  pub fn new(rect: Rectangle, rq: &mut RenderQueue, context: &mut Context) -> Result<Home, Error> {
  ...
  fn update_shelf(&mut self, was_resized: bool, rq: &mut RenderQueue) {
    let dpi = CURRENT_DEVICE.dpi;
    ...
    if was_resized {
      let page_position = if self.visible_books.is_empty() {
        ...
        self.current_page as f32 * (shelf.max_lines as f32 / self.visible_books.len() as f32)
      };
  ...
  fn toggle_search_bar(
    ...
    if update {
      ...
      if search_visible {
        ...
        self.update_shelf(true, &mut RenderQueue::new());
        rq.add(RenderData::new(
  ...
  fn toggle_book_menu(
    ...
    if let Some(index) = locate_by_id(self, ViewId::BookMenu) {
...

      {
        ...
        let submenu = [IntermKind::Suspend, IntermKind::PowerOff, IntermKind::Share]
          .iter()
          .map(|k| {
            EntryKind::CheckBox(
              k.label().to_string(),
  ...
  fn add_document(&mut self, mut info: Info, rq: &mut RenderQueue, context: &mut Context) {
    ...
    self.background_fetchers.retain(|dir, fetcher| {
      if dir == path {
        ...
        if let Some(first_column) = fetcher.first_column {
          hub
            .send(Event::Select(EntryId::FirstColumn(first_column)))
            .ok();
        }
        if let Some(second_column) = fetcher.second_column {
          hub
            .send(Event::Select(EntryId::SecondColumn(second_column)))
            .ok();
        }
        false
      } else {
  ...
  fn insert_fetcher(&mut self, hook: &Hook, hub: &Hub, context: &Context) {
    ...
    if let Some(second_column) =
      second_column.replace(context.settings.libraries[selected_library].second_column)
    {
      hub
        .send(Event::Select(EntryId::SecondColumn(second_column)))
        .ok();
    ...
    let process = hook.program.as_ref().and_then(|p| {
      ...
      self
        .spawn_child(&dir, p, context.settings.wifi, context.online, hub)
        .map_err(|e| eprintln!("Can't spawn child: {}.", e))
        .ok()
    });
  ...
  fn handle_event(
    ...
    match *evt {
      ...
      Event::AddDocument(ref info) => {
        ...
        self.add_document(*info2, rq, context);
        true
      },
      ...
      Event::SelectDirectory(ref path) | Event::Select(EntryId::SelectDirectory(ref path)) => {
        self.select_directory(path, hub, rq, context);
        true
      },
