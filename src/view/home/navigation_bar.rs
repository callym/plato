...
fn guess_bar_size(dirs: &BTreeSet<PathBuf>) -> usize {
  (dirs.iter().map(|dir| dir.as_os_str().len()).sum::<usize>() / 300)
    .max(1)
    .min(4)
}
        ...
        match dir {
          Dir::North | Dir::South => {
            ...
            if let Some(index) = self
              .children
              .iter()
              .position(|child| child.is::<DirectoriesBar>() && child.rect().includes(pt))
            {
