...
impl DirectoriesBar {
  ...
  fn make_children(
    ...
    if page.end_index == directories_count {
      ...
      let height =
        big_half(baselines[lines_count - 1]) + x_height + small_half(baselines[lines_count]);
      let filler = Filler::new(
        rect![
