...
use crate::{
  app::Context,
  color::{KEYBOARD_BG, TEXT_INVERTED_HARD, TEXT_NORMAL},
  device::CURRENT_DEVICE,
  font::{font_from_style, Fonts, KBD_CHAR, KBD_LABEL},
  framebuffer::{Framebuffer, UpdateMode},
  geom::{CornerSpec, LinearDir, Rectangle},
    ...
    match *evt {
      ...
      Event::Gesture(GestureEvent::HoldFingerShort(center, ..)) if self.rect.includes(center) => {
        match self.kind {
          KeyKind::Delete(dir) => {
            hub
              .send(Event::Keyboard(KeyboardEvent::Delete {
          ...
          KeyKind::Move(dir) => {
            hub
              .send(Event::Keyboard(KeyboardEvent::Move {
                target: TextKind::Word,
                dir,
              }))
              .ok();
