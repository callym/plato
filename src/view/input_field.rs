...
use crate::{
  ...
  font::{font_from_style, Fonts, FONT_SIZES, NORMAL_STYLE},
  framebuffer::{Framebuffer, UpdateMode},
  geom::{halves, BorderSpec, LinearDir, Point, Rectangle},
  ...
  fn handle_event(
    ...
    match *evt {
      ...
      Event::Gesture(GestureEvent::HoldFingerShort(center, _)) if self.rect.includes(center) => {
        hub
          .send(Event::ToggleInputHistoryMenu(self.view_id, self.rect))
          .ok();
        true
      },
      Event::Focus(id_opt) => {
