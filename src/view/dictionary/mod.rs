...
use crate::{
  ...
  document::{html::HtmlDocument, Document, Location},
  font::Fonts,
  framebuffer::{Framebuffer, Pixmap, UpdateMode},
  geom::{halves, CycleDir, Dir, Point, Rectangle},
...
impl Dictionary {
  pub fn new(
    ...
    let bottom_bar = BottomBar::new(
      ...
      target.as_ref().map(String::as_str).unwrap_or("All"),
      false,
      false,
    );
    ...
    if query.is_empty() {
      hub
        .send(Event::Focus(Some(ViewId::DictionarySearchInput)))
        .ok();
    } else {
  ...
  fn toggle_edit_languages(
    ...
    if let Some(index) = locate_by_id(self, ViewId::EditLanguages) {
      ...
      hub
        .send(Event::Focus(Some(ViewId::EditLanguagesInput)))
        .ok();
