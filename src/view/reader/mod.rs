...
impl Reader {
  ...
  fn go_to_chapter(&mut self, dir: CycleDir, hub: &Hub, rq: &mut RenderQueue, context: &Context) {
    ...
    let loc = {
      ...
      if let Some(toc) = self.toc().or_else(|| doc.toc()) {
        ...
        chap_offset.or_else(|| {
          doc
            .chapter_relative(current_page, dir, &toc)
  ...
  fn toggle_text_align_menu(
    ...
    if let Some(index) = locate_by_id(self, ViewId::TextAlignMenu) {
      ...
      let entries = choices
        .iter()
        .map(|v| EntryKind::RadioButton(v.to_string(), EntryId::SetTextAlign(*v), text_align == *v))
        .collect();
  ...
  fn find_page_by_name(&self, name: &str) -> Option<usize> {
    self.info.reader.as_ref().and_then(|r| {
      if let Ok(a) = u32::from_str_radix(name, 10) {
        r.page_names
          .iter()
          .filter_map(|(i, s)| u32::from_str_radix(s, 10).ok().map(|b| (b, i)))
          .filter(|(b, _)| *b <= a)
  ...
  fn handle_event(
    ...
    match *evt {
      ...
      Event::Gesture(GestureEvent::Spread {
        ...
        starts,
        ..
      }) if self.rect.includes(starts[0]) => {
        if !self.reflowable {
      ...
      Event::Select(EntryId::GoToSelectedPageName) => {
        self
          ...
          .and_then(|text| {
            let end = text
              .find(|c: char| {
                !c.is_ascii_digit() && !Digit::from_char(c).is_ok() && !c.is_ascii_uppercase()
              })
              .unwrap_or(text.len());
