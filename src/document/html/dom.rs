...
impl Node {
  ...
  pub fn text(&self) -> Option<&str> {
    match *self {
      Node::Text(TextData { ref text, .. }) | Node::Whitespace(TextData { ref text, .. }) => {
        Some(text)
      },
  ...
  pub fn is_block(&self) -> bool {
    match *self {
      Node::Element(ElementData { ref name, .. }) => match name.as_str() {
  ...
  pub fn wrap_lost_inlines(&mut self) {
    if let Node::Element(ElementData {
      ref mut children, ..
    }) = self
    {
      if children.iter().any(Self::is_block) {
        ...
        while i < children.len() {
          ...
          if (is_block || i == children.len() - 1) && start_index.is_some() {
            ...
            if let Some(Node::Element(ElementData {
              ref mut children, ..
            })) = children.get_mut(j)
            {
              *children = inlines;
