use super::{
  ...
  style::{specified_values, Stylesheet},
};
use crate::{
  document::{pdf::PdfOpener, Document, Location},
  font::{FontFamily, FontOpener},
  framebuffer::{Framebuffer, Pixmap},
  geom::{Edge, Rectangle},
  helpers::{decode_entities, Normalize},
  settings::{DEFAULT_FONT_SIZE, DEFAULT_LINE_HEIGHT, DEFAULT_MARGIN_WIDTH, DEFAULT_TEXT_ALIGN},
  unit::{mm_to_px, pt_to_px},
};
use anyhow::Error;
  ...
  pub fn build_display_list(
    ...
    match node.tag_name() {
      ...
      Some("table") => {
        ...
        draw_state.center_table =
          style.display == Display::InlineTable && parent_style.text_align == TextAlign::Center;
    ...
    style.line_height = props
      ...
      .unwrap_or_else(|| {
        ((style.font_size / parent_style.font_size) * parent_style.line_height as f32).round()
          as i32
      });
    ...
    if width < 0 {
      if style.width > 0 {
        let total_space =
          style.margin.left + style.padding.left + style.margin.right + style.padding.right;
    ...
    let has_blocks = node.children().and_then(|children| {
      children
        .iter()
        .skip_while(|child| child.is_whitespace())
        .next()
        .map(Node::is_block)
    });
          ...
          if draw_state.column_widths.is_empty() {
            ...
            if min_row_width >= width {
              draw_state.column_widths = draw_state
                .min_column_widths
                .iter()
                .map(|w| ((*w as f32 / min_row_width as f32) * width as f32).round() as i32)
                .collect();
        ...
        if children.is_empty() {
          ...
          if !inlines.is_empty() {
            draw_state.prefix = match style.list_style_type {
              None => match loop_context.parent.and_then(|parent| parent.tag_name()) {
          ...
          let min_width = display_list
            ...
            .filter_map(|dc| match dc {
              ...
              DrawCommand::Image(ImageCommand { rect, .. }) => Some(
                (rect.width() as i32)
                  .min(pt_to_px(parent_style.font_size, self.dpi).round().max(1.0) as i32)
                  + horiz_padding,
              ),
              _ => None,
            })
  ...
  fn place_paragraphs(
    ...
    for mut element in floats.into_iter() {
      ...
      if let Some(ref mut floating_rects) = draw_state.floats.get_mut(&page_index) {
        if let Some(orect) = floating_rects
          .iter()
          .rev()
          .find(|orect| orect.max.y > y_min && (orect.min.x - style.start_x).signum() == side)
        {
          y_min = orect.max.y;
        }
      }
    ...
    if let Some(prefix) = draw_state.prefix.as_ref() {
      ...
      let prefix_plan = {
        ...
        font.plan(
          prefix,
          None,
          style.font_features.as_ref().map(Vec::as_slice),
        )
      };
    ...
    for bp in bps {
              ...
              ParagraphElement::Image(element) => {
                ...
                let (w, h, pt, scale) = if k == index {
                  ...
                  if position.y + height > root_data.rect.max.y - space_bottom {
                    let mut ratio =
                      (root_data.rect.max.y - position.y - space_bottom) as f32 / height as f32;
                    if ratio < 0.33 {
                      display_list.push(page);
                      position.y = root_data.rect.min.y;
                      page = Vec::new();
                      ratio = ((root_data.rect.max.y - position.y - space_bottom) as f32
                        / height as f32)
                        .min(1.0);
                    }
      ...
      if let ParagraphItem::Penalty { width, .. } = items[index] {
        if width > 0 {
          ...
          if let Some(DrawCommand::Text(TextCommand {
            ref mut rect,
            ref mut plan,
            ref mut text,
            ..
  ...
  fn box_from_chunk(
    ...
    let mut plan = {
      ...
      font.plan(
        chunk,
        None,
        element.font_features.as_ref().map(Vec::as_slice),
      )
    };
      ...
      match itm {
        ParagraphItem::Box {
          ...
          let hyphen_width = if dictionary.is_some() {
            ...
            font.set_size(element.font_size, self.dpi);
            font
              .plan("-", None, element.font_features.as_ref().map(Vec::as_slice))
              .width
          } else {
            0
          };
          for (end_index, is_hardbreak) in LineBreakIterator::new(text) {
            ...
            if let Some(dict) = dictionary {
              let mut index_before = chunk
                .find(char::is_alphabetic)
                .unwrap_or_else(|| chunk.len());
              if index_before > 0 {
                let subelem = self.box_from_chunk(&chunk[0..index_before], start_index, &element);
                hyph_items.push(subelem);
              }

              let mut index_after = chunk[index_before..]
              ...
              while index_before < index_after {
                ...
                for segment in dict.hyphenate(subchunk).iter().segments() {
                  let subelem =
                    self.box_from_chunk(segment, start_index + index_before + index, &element);
                  hyph_items.push(subelem);
                  index += segment.len();
                ...
                index_before = chunk[index_after..]
                  .find(char::is_alphabetic)
                  .map(|i| index_after + i)
                  .unwrap_or_else(|| chunk.len());
                if index_before > index_after {
                  let subelem = self.box_from_chunk(
                    &chunk[index_after..index_before],
                ...
                index_after = chunk[index_before..]
                  .find(|c: char| !c.is_alphabetic())
                  .map(|i| index_before + i)
                  .unwrap_or_else(|| chunk.len());
              }
            ...
            if !is_hardbreak {
              let penalty = if chunk.ends_with('-') {
                HYPHEN_PENALTY
              } else {
                0
              };
        ...
        DrawCommand::Image(ImageCommand {
          ...
          if let Ok(buf) = resource_fetcher.fetch(path) {
            PdfOpener::new()
              ...
              .and_then(|mut doc| doc.pixmap(Location::Exact(0), *scale))
              .map(|(pixmap, _)| {
