use super::{
  css::{CssParser, Rule, Selector, SimpleSelector, Specificity},
  dom::Node,
};
...
mod tests {
  ...
  fn simple_style() {
    ...
    assert_eq!(
      specified_values(&xml1, None, None, &css1),
      [
    ...
    assert_eq!(
      specified_values(&xml2, None, None, &css1),
...
fn matching_rules<'a>(
  ...
  stylesheet
    .iter()
    .filter_map(|rule| match_rule(node, parent, sibling, rule))
    .collect()
}
...
fn match_rule<'a>(
  ...
  rule
    .selectors
    .iter()
    .find(|selector| matches(node, parent, sibling, *selector))
