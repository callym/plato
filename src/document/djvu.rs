...
use super::{chapter, chapter_relative, BoundedText, Document, Location, TextLocation, TocEntry};
use crate::{
  framebuffer::Pixmap,
...
impl Document for DjvuDocument {
  ...
  fn pixmap(&mut self, loc: Location, scale: f32) -> Option<(Pixmap, usize)> {
    ...
    self
      .page(index)
      .and_then(|page| page.pixmap(scale))
    ...
    unsafe {
      ...
      for i in 0..len {
        ...
        let digits = bytes
          ...
          .collect::<String>();
        let location = Location::Exact(digits.parse::<usize>().unwrap_or(1).saturating_sub(1));
