use super::html::{
  ...
  dom::Node,
  engine::{Engine, Page, ResourceFetcher},
  layout::{
    DrawCommand,
    DrawState,
    ImageCommand,
      ...
      for child in children {
        if child.tag_name() == Some("navPoint") {
          let title = child
            ...
            .and_then(|text| text.text().map(decode_entities).map(Cow::into_owned))
            .unwrap_or_default();

          // Example URI: pr03.html#codecomma_and_what_to_do_with_it
          let rel_uri = child
            .find("content")
            .and_then(|content| content.attr("src").map(String::from))
            .unwrap_or_default();
